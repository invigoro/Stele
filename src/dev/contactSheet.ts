...
import { defaultSettings, withText } from '../settings';
...
async function main(): Promise<void> {
  ...
  for (const medium of media) {
    ...
    const base = text ? { ...withText(start, { text }), textEdited: true } : start;
    ...
    const fonts = base.blocks.flatMap((block) => (block.kind === 'text' ? [block.font] : []));
    await Promise.all(fonts.map((font) => loadFont(FONTS[font])));
    const measure = (font: FontId) => measureFont(FONTS[font]);
