...
layout(location = 0) out vec4 outSurface; // height (mm), alpha, roughness, metal
layout(location = 1) out vec4 outAlbedo;  // sRGB colour

void main() {
  vec2 p = u_originMm + gl_FragCoord.xy / u_pxPerMm;
  Surface s = Surface(0.0, vec3(1.0), 0.5, 1.0, 0.0);
  buildSurface(p, s);
  outSurface = vec4(s.height, clamp(s.alpha, 0.0, 1.0), s.roughness, s.metal);
